<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold mb-2">Purchase Order Requested</h1>
    <div class="bg-white shadow-md rounded-lg p-8">
      <h2 class="text-xl font-semibold">Details:</h2>
      <p><strong>Order ID:</strong> {{ orderId }}</p>
      <p><strong>Name on Receipt:</strong> {{ billName }}</p>
      <p><strong>Amount:</strong> {{ currency }}{{ amount }}</p>
      <p class="mb-6">Thank You! We have confirmed your Purchase Order request. But before you go, we need a few details.</p>
...
export default {
  ...
  created() {
    ...
    this.billName = decodeURIComponent(urlParams.get('bill_name') || '');
    this.amount = urlParams.get('amount') || 0;
